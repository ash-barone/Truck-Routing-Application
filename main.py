...
if __name__ == '__main__':
    ...
    while isOpen:
        ...
        if option == "1":
            print("\n")
            set_time_hour = input("Please enter a specific hour: ")
            ...
            if set_time >= datetime.now().replace(hour=8, minute=0, second=0):
                try:
                    ...
                    for p in packages.get_all_packages():
                        '''for p in truck1.get_delivered_packages():
                            if temp_p.__str__() == p.__str__:
                                print(temp_p)
                        for p in truck1.get_loaded_packages():
                            if temp_p.__str__() == p.__str__:
                                print(temp_p)
                        for p in truck2.get_delivered_packages():
                            if temp_p.__str__() == p.__str__:
                                print(temp_p)
                        for p in truck2.get_loaded_packages():
                            if temp_p.__str__() == p.__str__:
                                print(temp_p)
                        for p in truck3.get_delivered_packages():
                            if temp_p.__str__() == p.__str__:
                                print(temp_p)
                        for p in truck3.get_loaded_packages():
                            if temp_p.__str__() == p.__str__:
                                print(temp_p)'''
        ...
        # allows user to look up a package using the package id
            ...
            packages.create_package_lists()
            if hash_table.search(lookup_id) is None:
                print("Package not found. Please try again.")
            else:
                print("Package: ")
                print(hash_table.search(lookup_id))
